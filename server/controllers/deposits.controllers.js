...
export const getDepositsByDate = async (req, res) => {
    try {
        const [result] = await pool.query("SELECT orders.id, orders.clientId, orders.items, orders.deposit, deposits.paymentMethod, deposits.depositValue, deposits.lastDeposit, deposits.newDeposit, CONVERT_TZ(deposits.depositCreatedAt, '+00:00', '-05:00'), clients.clientName, clients.premises, clients.mall as depositCreatedAt FROM deposits join orders on orders.id = deposits.orderId join clients on orders.clientId = clients.id WHERE DATE(CONVERT_TZ(deposits.depositCreatedAt, '+00:00', '-05:00')) = ? ORDER BY orders.clientId, deposits.depositCreatedAt, orders.createdAt ASC", [
            req.params.date,
        ]);
        if (result.length === 0)
